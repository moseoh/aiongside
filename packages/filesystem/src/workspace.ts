            ...
            postActions: [
              {
                ...
                workId: record.id,
                message: `Work completed. Decide whether Knowledge needs this Work's results, then report the decision to the user in one line before finishing: the updated key and what was added, or why no update is needed. Read work/${record.id}/record.md and its deliverables. Read knowledge/index.md and follow only relevant paths; use aiongside knowledge list or show to resolve document keys when needed. Compare the Work results with existing Knowledge. If reusable results need to be added or corrected, update an existing document or create one with aiongside knowledge new <key>, then write the content without duplicating existing material. If files or folders were added, moved, or removed, update the affected index.md routing links and related document links. After incorporating the Work results, record the contribution with aiongside work knowledge add ${record.id} <key> only if that relationship is absent. Do not add relationships for reference-only topics. If no update is needed, no further action is required beyond the report. The CLI does not edit Knowledge content or verify incorporation.`,
