...
Each agent session binds to one workspace at its first `SessionStart` (`startup` or `clear`). Later directory changes, nested workspaces such as `.legacy`, resume, and compaction do not change that binding. Stop checks the same workspace whose instructions were loaded. Recovery output identifies the workspace and the `aiongside --root '<workspace>'` prefix to use for its commands. Ordinary CLI commands keep their current-directory and explicit `--root` behavior. Use a separate session for another workspace.

Bindings are small private files under `$XDG_CACHE_HOME/aiongside/hook-sessions` (default `~/.cache/aiongside/hook-sessions`), keyed by a hash of the agent's `session_id`. Stop only reads them. They persist for session resume; there is no automatic expiry. A missing, damaged, or unavailable binding reports an error instead of checking another directory. After installing this behavior for the first time, start a new session in the intended workspace; old sessions have no binding. No workspace upgrade is required for this adapter-only change.
